...
class TestMungePoFile(PoxxTestCase):
    def test_munge(self):
        ...
        self.assertEqual(munged_pofile[0].msgstr, "Ẑḗřǿ")
        self.assertEqual(munged_pofile[1].msgstr, "Ǿƞḗ")
        self.assertEqual(munged_pofile[2].msgstr, "Ŧẇǿ")
        self.assertEqual(munged_pofile[3].msgstr, "Ŧħřḗḗ")
        self.assertEqual(
            munged_pofile[4].msgstr,
            "Ẑḗřǿ {name} Ǿƞḗ {confidence:0.2f} Ŧẇǿ Ŧħřḗḗ %(qty)0.2f Ƒǿŭř %(name)s Ƒīṽḗ %(age)d",
        )
        self.assertEqual(
            munged_pofile[5].msgstr,
            "Ẑḗřǿ Ǿƞḗ {age:3s} Ŧẇǿ Ŧħřḗḗ",
        )
    ...
    def test_blank_munge(self):
        ...
        self.assertEqual(munged_pofile[0].msgstr, "")
        self.assertEqual(munged_pofile[1].msgstr, "")
        self.assertEqual(munged_pofile[2].msgstr, "")
        self.assertEqual(munged_pofile[3].msgstr, "")
    ...
    def test_canonical_munge(self):
        ...
        self.assertEqual(munged_pofile[0].msgstr, "Ẑḗřǿ")
        self.assertEqual(munged_pofile[1].msgstr, "Uno")
        self.assertEqual(munged_pofile[2].msgstr, "Ŧẇǿ")
        self.assertEqual(munged_pofile[3].msgstr, "Tree")
...
class DiffTestCase(PoxxTestCase):
    ...
    def test_diff(self):
        ...
        self.assertEqual(diff_pofile[0].msgid, "Zero")
        self.assertEqual(diff_pofile[1].msgid, "Two")
